...
class LDM(nn.Module):
    def __init__(self, latent_dim=4, time_steps=1000, upscale_factor=4):
    ...
    def forward(self, x, t):
        ...
        # Upscale the output
        upscaled = self.upscale(noise_pred)
        
        # Reshape and permute for pixel shuffle
        b, c, h, w = upscaled.shape
        upscaled = upscaled.view(b, 3, self.upscale_factor, self.upscale_factor, h, w)
        upscaled = upscaled.permute(0, 1, 4, 2, 5, 3).contiguous()
        upscaled = upscaled.view(b, 3, h * self.upscale_factor, w * self.upscale_factor)
        
        return upscaledimport torch
import torch.nn as nn
import torch.nn.functional as F
import math
