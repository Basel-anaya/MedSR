...
class SwinIR(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=3, embed_dim=96,
        ...
        self.patch_embed = nn.Conv2d(in_channels=in_chans, out_channels=embed_dim, kernel_size=patch_size, stride=patch_size)

        self.patch_norm = norm_layer(embed_dim) if patch_norm else None
        ...
        if self.upsampler == 'pixelshuffledirect':
            self.upsample = nn.Sequential(
                nn.Conv2d(embed_dim, 4 * in_chans * upscale ** 2, 3, 1, 1),
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        x = x.transpose(1, 2).view(B, -1, H, W)  # B C Ph Pw
